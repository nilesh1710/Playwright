# Page snapshot

```yaml
- navigation:
  - list:
    - listitem:
      - link "KredSafe":
        - /url: /
        - img "KredSafe"
    - listitem:
      - img "KredSafe"
  - list:
    - listitem: "Previous Login: 2 hours ago"
    - listitem:
      - link "":
        - /url: "#"
    - listitem:
      - link " 1":
        - /url: https://dev.kredsafe.net/user/subscription/cart
    - listitem:
      - link "Sam Heyri Sam Heyri":
        - /url: "#"
        - img "Sam Heyri"
        - text: Sam Heyri
- complementary:
  - navigation:
    - list:
      - listitem:
        - link "Home":
          - /url: https://dev.kredsafe.net/user/home
          - img "Home":
            - img
      - listitem:
        - link:
          - /url: https://dev.kredsafe.net/user/overview
          - img:
            - img
      - listitem:
        - link:
          - /url: https://dev.kredsafe.net/user/documents
          - img:
            - img
      - listitem:
        - link "":
          - /url: https://dev.kredsafe.net/user/credentials
      - listitem:
        - link:
          - /url: https://dev.kredsafe.net/user/notes
          - img:
            - img
      - listitem:
        - link:
          - /url: https://dev.kredsafe.net/user/es_forms
          - img:
            - img
      - listitem:
        - link:
          - /url: https://dev.kredsafe.net/user/es-packets
          - img:
            - img
      - listitem:
        - link "":
          - /url: https://dev.kredsafe.net/user/subscriptions
- heading " Your shopping cart" [level=1]
- 'heading "Premium Plan - Tenure: " [level=2]':
  - strong: "Premium Plan - Tenure:"
  - link "":
    - /url: https://dev.kredsafe.net/user/subscription/cart/1120/remove
- heading "3 Years" [level=3]
- heading "Total amount to pay:" [level=2]:
  - strong: "Total amount to pay:"
- heading "$10,200.00" [level=3]
- paragraph: "You can pay using the following methods:"
- tablist:
  - tab "Credit Card "
- checkbox [checked]
- text: Save
- link " Edit Subscription":
  - /url: https://dev.kredsafe.net/user/subscription/cart/1120/edit
- button "Confirm & Pay"
- link "Terms and Conditions":
  - /url: "#"
- contentinfo: © Copyright by KredSafe. All Rights Reserved.
```